...
<script>
	...
	function modify(iconNo) {
		disableScreen();
		$.getJSON("info?iconNo=" + iconNo, function(json) {
			$("#editForm input[name=iconNo]").val(json.param.info.iconNo);
	...
	function remove(iconNo) {
		if(confirm("삭제하시겠습니까?")) {
			disableScreen();
			$("#editForm input[name=iconNo]").val(iconNo);
	...
	function uploadImage(field) {
		if($("input[name=" + field + "]").val() != '') {
	        ...
	        var action = "/upload/upload?field=" + field + "&path=icon&fileType=image";
...
<c:import url="/include/header" />
	<div id="container">
		...
		<form name="editForm" id="editForm" method="post" action="create">
			<input type="hidden" name="iconNo" value="">
		<div class="white_box">
			<table class="board">
				<colgroup>
					<col width="10%">
				...
				<tr>
					...
					<td>
						...
						<p class="txt">* Image Size : 40*40(jpg) / Max 10Mbyte</p>
			...
			<tbody id="subList">
				...
				<tr>
					...
					<td>
						...
						<input type="hidden" name="iconNo" value="${ row.iconNo }">
					</td>
					<td>
						<c:if test="${ updateAuth eq 'Y' }">
							<input type="button" class="btnTypeB btnSizeB" onclick="modify('${ row.iconNo }')" value="수정">
							<input type="button" class="btnTypeC btnSizeB" onclick="remove('${ row.iconNo }')" value="삭제">
						</c:if>
					</td>
				</tr>
				</c:forEach>
