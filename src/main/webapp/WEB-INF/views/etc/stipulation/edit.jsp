...
<c:import url="/include/header" />
	<div id="container">
		...
		<form name="editForm" id="editForm" method="POST" action="${ mode }">
			<input type="hidden" name="stipulationNo" value="${ stipulation.stipulationNo }" />
		<h2>정보 등록</h2>
		<div class="white_box">
			<table class="board">
				<colgroup>
				...
				<tr>
					<th>제목</th>
					<td>
						<input type="text" name="title" id="title" value="${ stipulation.title }" style="width:100%" maxlength="200">
					</td>
				</tr>
				<tr>
					<th>내용</th>
					<td>
						<textarea name="content" id="content" style="width:100%;height:100px">${ stipulation.content }</textarea>
					</td>
				</tr>
			</table>
		</div>
		...
		<div class="white_box">
			<table class="board">
				<tr>
					<th>공개여부</th>
					<td>
						<label><input type="radio" name="status" value="S" ${ stipulation.status eq 'S' ||  empty stipulation.status ? 'checked' : '' }><span>공개</span></label>
						<label><input type="radio" name="status" value="H" ${ stipulation.status eq 'H' ? 'checked' : '' }><span>비공개</span></label>
					</td>
					<th>등록일/등록자</th>
					<td><fmt:formatDate value="${ stipulation.cdate }" pattern="${ DateTimeFormat }" /> / ${stipulation.cuser}
					</td>
					<th>수정일/수정자</th>
					<td><fmt:formatDate value="${stipulation.udate}" pattern="${ DateTimeFormat }"/> / ${stipulation.uuser}
					</td>
				</tr>
			</table>
		</div>
