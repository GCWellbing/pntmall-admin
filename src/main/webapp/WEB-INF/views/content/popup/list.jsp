	...
	<div id="container">
		...
		<div class="white_box">
			<table class="board">
				...
				<tr>
					...
					<th>분류</th>
					<td>
						<label><input type="radio" name="type" value="" ${ empty param.type ? 'checked' : '' }><span>전체</span></label>
						<label><input type="radio" name="type" value="1" ${ param.type eq 1 ? 'checked' : '' }><span>상단띠배너</span></label>
						<label><input type="radio" name="type" value="2" ${ param.type eq 2 ? 'checked' : '' }><span>전면팝업</span></label>
					</td>
				</tr>
				<tr>
					<th>게시 기간</th>
		...
		<table class="list">
			...
			<tbody>
				...
				<tr>
					<td><c:out value="${ row.popupid }" /></td>
					<td><img src="${ row.pcImg }" id="img"></td>
					<td><a href="edit?popupid=${ row.popupid }"><c:out value="${ row.title }" /></a></td>
